...
describe("ReviewReportPage", () => {
  ...
  function respondWith(detail: ReportResponse, history: unknown[] = []) {
    fetchMock.mockImplementation((url: string, init: RequestInit) => {
      if (url.includes("/reports/7/review") && init.method === "POST") {
        return Promise.resolve(jsonResponse(200, { ...detail, status: "APPROVED" }));
      }
  ...
  function reviewCallBody() {
    const call = fetchMock.mock.calls.find(
      ([url, init]) => url.includes("/reports/7/review") && init.method === "POST",
    );
  ...
  it("approves a submitted report and redirects to /projects", async () => {
    ...
    await waitFor(() => expect(reviewCallBody()).toEqual({ action: "APPROVED", comment: null }));
  ...
  it("blocks a change request with no comment before calling the API", async () => {
    ...
    fireEvent.click(await screen.findByRole("radio", { name: "Request changes" }));
    fireEvent.click(screen.getByRole("button", { name: "Request changes" }));

    expect(await screen.findByText(/Explain what needs to change/i)).toBeInTheDocument();
  ...
  it("shows a non-SUBMITTED report read-only with no review actions", async () => {
    respondWith(
      report({ status: "APPROVED", content: { ...EMPTY_CONTENT, tasksPlannedNext: "Ship the API" } }),
    );
    ...
    expect(screen.getByText(/review actions are only available/i)).toBeInTheDocument();
  });
});
