...
export default function AdminUsersPage() {
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-8 p-6">
      ...
      {users.isPending ? <p className="text-sm text-gray-500">Loading users…</p> : null}
      {users.isError ? (
      ...
      {users.isSuccess ? (
        <>
          <PendingApprovalsSection users={users.data.filter((user) => user.status === "PENDING")} />
          <ActiveUsersSection users={users.data.filter((user) => user.status === "ACTIVE")} />
          <CreateUserForm />
        </>
      ) : null}
    </main>
